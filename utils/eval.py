...
def eval_corpus500_by_bert(input_eval_path=eval_data_path, output_eval_path='', verbose=True):
    from pycorrector.bert.deberta_corrector import BertCorrector
    model = BertCorrector()
    res = []
    corpus = load_json(input_eval_path)
    total_count = 0
...
deberta测试
'''
def eval_sighan_2015_by_deberta(sighan_path=sighan_2015_path, verbose=True, num_limit_lines=100):
    from pycorrector.bert.deberta_corrector import BertCorrector
    model = BertCorrector()
    total_count = 0
    right_count = 0
